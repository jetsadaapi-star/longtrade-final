    ...
    const fetchProducts = async () => {
        ...
        try {
            ...
            const response = await fetch(
                `${process.env.NEXT_PUBLIC_API_URL}/products/category/scripts?${params}`
            );
            ...
            <div className="max-w-7xl mx-auto">
                ...
                <div className="text-center mb-12">
                    ...
                    <p className="text-white/60 text-lg max-w-2xl mx-auto">
                        TradingView ช่วยเหลือการเทรด เพิ่มประสิทธิภาพและความสะดวกในการทำงาน
                    </p>
                </div>
                ...
                <div className="glass rounded-2xl border border-white/10 p-6 mb-8">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div className="relative">
                            ...
                            <input
                                type="text"
                                placeholder="ค้นหา TradingView..."
                                value={searchQuery}
                                onChange={(e) => setSearchQuery(e.target.value)}
                                className="w-full bg-white/5 border border-white/10 rounded-xl pl-10 pr-4 py-3 text-white placeholder:text-white/40 focus:outline-none focus:border-yellow-500/50 transition-colors"
                            />
                        </div>
                        ...
                        <select
                            ...
                            className="bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-yellow-500/50 transition-colors"
                        >
                ...
                {loading ? (
                    ...
                    <div className="text-center py-16">
                        ...
                        <p className="text-white/60 text-lg">ไม่พบ TradingView</p>
                    </div>
                ) : (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {products.map((product) => (
                            <Link
                                ...
                                className="glass rounded-2xl border border-white/10 p-6 hover:border-yellow-500/50 transition-all duration-300 hover:scale-[1.02] group"
                            >
                                {/* Thumbnail */}
                                <div className="aspect-video bg-gradient-to-br from-yellow-500/10 to-yellow-600/10 rounded-xl mb-4 overflow-hidden relative">
                                    {product.thumbnail ? (
                                        <img
                                        ...
                                        <div className="flex items-center justify-center h-full">
                                            <Code className="h-12 w-12 text-yellow-400/40" />
                                        </div>
                                    )}
                                    {product.is_featured && (
